...
First [read the documentation on queries](https://firebase.google.com/docs/firestore/query-data/queries). As you now know, Firestore queries are built from a collection reference. In Firemore queries are built by adding a query map to the end of a reference vector.
