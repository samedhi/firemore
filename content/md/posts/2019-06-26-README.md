...
(go
  (let [->output (->output-fx)
        ...
        luke-reference [:users user-id :characters "luke-example-1"]]
...
this database is being used by everyone currently reading this documentation. The
...
(go
  (let [->output (->output-fx)
        user-id (async/<! (firemore/uid))
        luke-reference [:users user-id :characters "luke-example-2"]]
        
    ;; Write Luke so we have something to watch
    (firemore/write! luke-reference initial-luke)
  
    (let [luke-chan (firemore/watch luke-reference)]
      ;; Luke's journey starts off as a farmboy
      (->output "Teenage Luke" (async/<! luke-chan))

      ;; Changing Luke's adult occupation...
      (async/<! (firemore/write! luke-reference (assoc initial-luke :occupation "jedi")))
      (->output "Adult Luke" (async/<! luke-chan))

      ;; Changing to Lukes final occupation after episode 7
      (async/<! (firemore/write! luke-reference (assoc initial-luke :occupation "One with the Force")))
      (->output "Episode 8 Luke" (async/<! luke-chan))

      ;; Remove luke from the Firestore database
      (async/<! (firemore/delete! luke-reference))
