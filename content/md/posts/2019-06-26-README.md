...
1. A sensible Authentication solution for managing your users.
...
The above code simply sets our current namespace to `firemore.readme` as well 
as requiring a few libraries we are going to need for the following examples. We will 
need the `cljs.core.async` libraries because we are doing asynchronous things across
the network and we want to make our code sane. 
...
We need firemore because... well, this is the walkthrough for the Firemore library.
...
We also declare some `initial-luke` data that we will use as a document in the following examples.
...
bunch of functions attached to it. A Firemore reference is a vector of
...
Keywords and strings are interchangeable in a reference. I prefer to use keywords
...
Does Luke Skywalker have Force powers? I can never remember? Let's check!
```language-klipse
(go
  (let [->output (->output-fx)
    ...
    ;; Write luke into the database so we have something to read.
    (async/<! (firemore/write! luke-reference initial-luke))

    ;; Then read luke back from the database and print the result
    (->output "luke" (async/<! (firemore/get luke-reference)))))
...
so you and only you may read and write to the location
...
Firemore attempts to provide useful defaults for converting the json like 
Firestore document to a clojure like Firemore document. 
...
But what if I want to see Luke change over time? What if I want to observe
Luke's Heroic Journey? Rather than getting Luke once, let's watch him.

```language-klipse
(async/go
  (let [user-id (async/async/<! (firemore/uid))
        luke-reference [:users user-id :characters "luke"])]
        
  ;; Write Luke so we have something to watch
  (write! luke-reference inital-luke)
  
  
  (let [luke-chan (watch luke-reference)]
    (println "Initial Luke ->" initial-luke)
    (assert initial-luke (async/<! luke-chan))

    (println "Merging in Luke's teenage occupation...")
    (async/<! (merge! luke-reference {:occupation "farmboy"}))
    (println "teenage Luke ->" (async/<! luke-chan))

    (println "Changing Luke's adult occupation...")
    (async/<! (write! luke-reference (assoc initial-luke :occupation "jedi")))
    (println "adult Luke ->" (async/<! luke-chan))

    (println "Changing to Lukes final occupation after episode 7")
    (async/<! (write! luke-reference (assoc initial-luke :occupation "One with the Force"})))
    (println "Episode 7 Luke ->" (async/<! luke-chan))

    (println "Remove luke from the Firestore database")
    (async/<! (delete! luke-reference))
    (println "Removed Luke reference ->" (async/<! luke-chan))
...
`luke-reference`, I get the updated version of Luke `put!` upon the `luke-chan`.
...
Firemore queries are built by adding a query map to the end of the reference vector.
